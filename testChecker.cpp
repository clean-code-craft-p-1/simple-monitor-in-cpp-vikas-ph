...
void TestVitalsMonitor::TestBodyTemparature() {
  ...
  assert(!VitalsMonitor::isBodyTemperatureNotOk(99));
  assert(!VitalsMonitor::isBodyTemperatureNotOk(98.1));
}

void TestVitalsMonitor::TestPulseRate() {
  assert(!VitalsMonitor::pulseRateOk(102));
  assert(VitalsMonitor::pulseRateOk(70));
  assert(VitalsMonitor::isPulseRateNotOk(102));
  assert(!VitalsMonitor::isPulseRateNotOk(70));
}
